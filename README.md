...
**TempFS** é um projeto que foi desenvolvido inicialmente como um prototipo para contêineres temporarios (`tempfs`) orquestrado para modelos GGUF usando [`llma-cpp`](https://github.com/ggml-org/llama.cpp).
...
O volume `llama_models` será usado para armazenar o modelo GGUF (configurado em [`config.json`](/config.json)), que será baixado automaticamente na primeira execução do `llama-cpp.js`.

## Uso Prático

O projeto oferece dois exemplos principais de ambientes:

### Servidor Llama-CPP ([`llama-cpp.js`](/llama-cpp.js))
...
### Ambiente Ubuntu Efêmero ([`tempfs.js`](/tempfs.js))
